...
# ============================================================
    ...
    **攻击链 (Attack Chain)** 是将孤立的异常事件串联起来形成的完整攻击路径。
    ...
    with st.expander("📖 攻击类型说明 — 点击展开", expanded=False):
        ...
        |---------|--------|------|---------|-------------|
        | **C2信标** | C2 Beacon | 受害主机与C2服务器建立周期性通信信道，攻击者通过此信道远程控制受害主机 | 周期性外联、固定间隔通信、心跳包模式 | T1071, T1573 |
        | **横向移动** | Lateral Movement | 攻击者通过已攻陷主机向内部网络扩散，利用跳板机访问更多内网目标 | 认证+网络连接序列、凭据复用、SMB/RDP/SSH | T1021, T1078 |
        | **数据外泄** | Data Exfiltration | 敏感数据被秘密传输到外部控制的服务器，通常发生在攻击末期 | 大流量出站、罕见外部目标、非标准端口 | T1041, T1048 |
        | **DGA活动** | DGA Activity | 恶意软件通过算法自动生成大量域名，用于定位C2服务器并绕过黑名单 | 高熵域名、大量NXDOMAIN响应、短域名生命周期 | T1568 |
        | **侦察扫描** | Recon Scan | 攻击者对目标网络进行大规模端口/服务扫描，为后续攻击收集信息 | 一对多连接、多端口探测、SYN扫描特征 | T1046, T1595 |
        | **可疑活动** | Suspicious Activity | 不符合已知攻击模式但风险评分较高的异常行为，需要人工进一步分析 | 通信模式罕见、路径稀有度高 | 待确认 |
        """)
        st.caption("每条攻击链都会自动映射到 MITRE ATT&CK 框架的战术技术编号，便于与威胁情报和合规要求对齐。")
    chains = DATA["chains"]
    if not chains:
